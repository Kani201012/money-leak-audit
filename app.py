...
if selected == "Home":
    ...
    with c2:
        st.image("https://cdn-icons-png.flaticon.com/512/3094/3094851.png", width=100)
    ...
    if run_btn and keyword and location:
        with st.spinner("Harvesting Market Data..."):
            ...
            if not raw_leads:
                ...
                # Inject Location into lead data for currency persistence
            ...
            with st.container():
                ...
                with col_a:
                    if st.button(f"📥 Add to Pipeline", key=f"add_{lead['place_id']}"):
                        ...
                        lead['currency'] = currency_symbol # Save currency to DB
                        if add_lead(lead):
                            st.toast(f"Target Acquired!", icon="✅")
                        else:
                            st.toast("Target already in pipeline.", icon="⚠️")
                with col_b:
                    # Pass currency_symbol to PDF Generator
    ...
    if not db:
        ...
        for lead in db:
            ...
            with st.expander(f"{lead['business_name']} | Gap: {currency_symbol}{lead.get('monthly_gap', 0):,}", expanded=False):
                ...
                with p1:
                    new_status = st.selectbox("Current Phase", ["New Lead", "Outreach Sent", "Negotiation", "Closed Won", "Lost"], index=["New Lead", "Outreach Sent", "Negotiation", "Closed Won", "Lost"].index(lead.get('status', "New Lead")), key=f"status_{lead['place_id']}")
                ...
                # GENERATE REPORT FROM PIPELINE
                # We need to re-run audit logic here to generate the PDF on demand
...
if selected == "Mission Control":
    ...
    if db:
        df = pd.DataFrame(db)
        st.dataframe(df[['business_name', 'status', 'rating', 'monthly_gap']], use_container_width=True)
    else:
